...
| DeleteObjects (batch) | Supported | A bucket-level `POST ?delete` parses the `<Delete>` XML strictly — doctypes, comments, CDATA, unknown elements, unresolved entities, more than 1000 objects, and versioned or conditional entries fail closed — authorizes every named key against `delete` grants (the first uncovered key rejects the whole batch before dispatch), then forwards the body unchanged so client checksums stay valid. The origin's per-key `Deleted`/`Error` XML passes back unchanged, and a confirmed outcome invalidates every requested key because quiet-mode responses omit per-key results. |
