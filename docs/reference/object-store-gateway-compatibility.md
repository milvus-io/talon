...
| DeleteObjects (batch) | Supported | A bucket-level `POST ?delete` parses the `<Delete>` XML strictly — doctypes, comments, CDATA, unknown elements, malformed character references, more than 1000 objects, and versioned or conditional entries fail closed — authorizes every named key against `delete` grants (the first uncovered key rejects the whole batch before dispatch), then forwards the body unchanged so client checksums stay valid. The origin's per-key `Deleted`/`Error` XML passes back unchanged, and a confirmed outcome invalidates every requested key because quiet-mode responses omit per-key results. The body is bounded at 8 MiB independently of the object-upload limit, since it must be buffered to authorize and forwarded verbatim. |
...
| Object sub-resources (`tagging`, `acl`, `versionId`, ...) | Rejected | A query parameter selects a different S3 operation rather than modifying the path's, so an unrecognized one is refused with `NotImplemented` for every method and target instead of being dropped. Dropping `?tagging` from a `DELETE` would destroy the object the client asked to keep, and dropping `?versionId` would act on the current version. Reserved `x-amz-*` parameters are not sub-resources and still carry presigned credentials. `response-*` header overrides are refused for the same reason; no Talon client sends them. |
